...
## 从动画数据创建 SpriteFrames
## @param animations_data: 动画配置数据 (包含 "anims" key 的 Dictionary)
## @return SpriteFrames: 创建的 SpriteFrames 对象
func create_sprite_frames(animations_data: Dictionary) -> SpriteFrames:
    ...
    for anim in animations_data["anims"]:
        var anim_key = anim["key"]
        ...
        # Set animation loop (default true, attack and die animations should not loop)
        var should_loop = true
        if "beam_saber_slash" in anim_key or "die" in anim_key or "rifle_shoot" in anim_key or "hurt" in anim_key:
            should_loop = false
        sprite_frames.set_animation_loop(anim_key, should_loop)
        
        # Calculate total animation duration and set FPS
        # In Godot, we need to set the animation speed (FPS) and then use frame durations as multipliers
        var total_duration = 0.0
        for frame in frames:
            total_duration += frame["duration"]
        
        # Set animation speed: frame_count / total_duration = FPS
        var frame_count = frames.size()
        var animation_fps = frame_count / total_duration if total_duration > 0 else 10.0
        sprite_frames.set_animation_speed(anim_key, animation_fps)
        
        # Add frames with relative durations
        for frame in frames:
            var frame_key = frame["key"]
            var frame_duration = frame["duration"]
            var texture = NetworkResourceLoader.get_cached_resource(frame_key)
            if texture:
                # Frame duration as a multiplier relative to the animation FPS
                # duration_multiplier = (frame_duration / total_duration) * frame_count
                var duration_multiplier = (frame_duration / total_duration) * frame_count
                sprite_frames.add_frame(anim_key, texture, duration_multiplier)
