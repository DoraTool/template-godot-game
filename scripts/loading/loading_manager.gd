...
var resources_to_load: Array[Dictionary] = []
var loaded_count: int = 0
var failed_count: int = 0
var total_count: int = 0

# Path to asset pack JSON file
...
func _load_resources_from_json() -> void:
		...
		for file_info in files:
			if file_info is Dictionary:
				...
				if resource_entry["url"] != "" and resource_entry["key"] != "":
					resources_to_load.append(resource_entry)
	...
	if total_count > 0:
		...
		for i in range(min(3, resources_to_load.size())):
			var res = resources_to_load[i]
			print("Resource %d: key=%s, type=%s" % [i, res["key"], res["type"]])
		
		# Show category breakdown
		var category_count: Dictionary = {}
		for res in resources_to_load:
			var cat = res.get("category", "unknown")
			category_count[cat] = category_count.get(cat, 0) + 1
		
		print("Resource categories: %s" % str(category_count))
	else:
...
func start_loading() -> void:
	...
	# Start loading all resources
	for resource_info in resources_to_load:
